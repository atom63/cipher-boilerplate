 ...
 */
export function VerticalLines({ columns = 4, className }: VerticalLinesProps) {
  const lines = columns + 1

  return (
    <div
      aria-hidden="true"
      className={cn('pointer-events-none flex h-full justify-between', className)}
    >
      {Array.from({ length: lines }, (_, i) => (
        <div className="h-full w-px bg-current" key={i} />
      ))}
    </div>
  )
}
